                                 ...
                                 const std::string& filename)
{
    ...
    if (ancho <= 0 || alto <= 0) {
        std::cerr << "[Imagen] Error: dimensiones inválidas (" 
    ...
    if (!file) {
        std::cerr << "[Imagen] Error: no se pudo crear el archivo '" 
                  << filename << "'." << std::endl;
        return;
    }
    ...
    if (!file) {
        std::cerr << "[Imagen] Error: fallo al escribir el archivo '" 
                  << filename << "'." << std::endl;
    } else {
        std::cout << "[Imagen] Imagen guardada correctamente: " 
                  << filename << std::endl;
    }
}
