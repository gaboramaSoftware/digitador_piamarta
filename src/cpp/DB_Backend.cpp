...
bool DB_Backend::Inicializar_DB() {
  ...
  bool DB_Backend::Guardar_Configuracion(const Configuracion &config) {
    ...
    sqlite3_bind_text(stmt, 1, config.id_terminal.c_str(), -1,
                      SQLITE_TRANSIENT);
  ...
  bool DB_Backend::Verificar_Racion_Doble(const std::string &run_id,
    ...
    try {
      int rc;
      if (sqlite3_prepare_v2(db_handle_, sql, -1, &stmt, nullptr) !=
          SQLITE_OK) {
  ...
  bool DB_Backend::Guardar_Registro_Racion(const RegistroRacion &registro) {
    ...
    const char *sql = "INSERT INTO RegistrosRaciones "
                      "(id_estudiante, fecha_servicio, tipo_racion, "
  ...
  std::vector<RegistroRacion> DB_Backend::Obtener_Registros_Pendientes() {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      ...
      r.tipo_racion =
          to_tipo_racion(tipo).value_or(TipoRacion::RACION_NO_APLICA);
  ...
  bool DB_Backend::Enrolar_Estudiante_Completo(
    ...
    try {
      ...
      // --- 1. INSERTAR USUARIO ---
      const char *sql_user =
          "INSERT INTO Usuarios (run_id, dv, nombre_completo, id_rol, "
          "template_huella, password_hash) "
          "VALUES (?1, ?2, ?3, ?4, ?5, ?6);";
