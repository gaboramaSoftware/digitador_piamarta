...
bool DB_Backend::Obtener_Todos_Templates(
    ...
    const char* sql = "SELECT run_id, template_huella FROM Usuarios WHERE template_huella IS NOT NULL;";
    ...
    sqlite3_finalize(stmt);
    std::cout << "[DB_Backend] Se cargaron " << count << " templates en memoria.\n";
    return true;
}
...
RolUsuario DB_Backend::Obtener_Perfil_Por_Run(const std::string& run_id,PerfilEstudiante& out_perfil){
  std::lock_guard<std::mutex> lock(db_mutex_);
...
bool DB_Backend::Guardar_Registro_Racion(const RegistroRacion& registro){
    ...
    sqlite3_bind_text(stmt, 1, registro.id_estudiante.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, registro.fecha_servicio.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_int (stmt, 3, to_int(registro.tipo_racion));
    sqlite3_bind_text(stmt, 4, registro.id_terminal.c_str(), -1, SQLITE_TRANSIENT);
    ...
    if(!exito) {
        ...
        std::cout << "[+ DB_BACKEND]: racion registrada para estudiante: " << registro.id_estudiante << "\n"; 
    }
...
std::vector<RegistroRacion> DB_Backend::Obtener_Registros_Pendientes() {
    ...
    const char* sql = 
        "SELECT id_registro, id_estudiante, fecha_servicio, tipo_racion, id_terminal, hora_evento "
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        r.fecha_servicio  = txt_fecha ? reinterpret_cast<const char*>(txt_fecha) : "";
...
bool DB_Backend::Marcar_Registros_Sincronizados(
    ...
    std::lock_guard<std::mutex> lock(db_mutex_);
    if (ids_registros.empty()) return true;

    _ejecutar_sql_script("BEGIN TRANSACTION;");

    const char* sql = "UPDATE RegistrosRaciones SET estado_registro = 1 WHERE id_registro = ?1;";
    sqlite3_stmt* stmt = nullptr;
...
bool DB_Backend::Enrolar_Estudiante_Completo(
    ...
    try {
        ...
        sqlite3_bind_text(stmt_user, 3, request.nombre_nuevo.c_str(), -1, SQLITE_TRANSIENT);
        ...
        const char* sql_details =
            "INSERT INTO DetallesEstudiante (run_id, curso) "
            "VALUES (?1, ?2);";
...
bool DB_Backend::Eliminar_Usuario(const std::string& run_id) {
    ...
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_handle_, sql, -1, &stmt, nullptr) != SQLITE_OK) return false;
...
bool DB_Backend::borrarTodo() {
    ...
    const char* sql_clear =
        "DELETE FROM RegistrosRaciones;"
        "DELETE FROM DetallesEstudiante;"
        "DELETE FROM Usuarios;";

    std::cout << "[DB_Backend][QA] Borrando todos los datos de tablas principales...\n";
