...
void guardarTemplateEnArchivo(const std::vector<unsigned char>& templateData){
    ofstream archivo("mi_huella.tpl", ios::binary);
    if (archivo) {
        archivo.write(reinterpret_cast<const char*>(templateData.data()), templateData.size());
        archivo.close();
        cout << "(Respaldo 'mi_huella.tpl' creado en disco)" << endl;
    }
}
...
int main(){
    ...
    // 1. Crear una instancia del sensor
    Sensor miSensor;
